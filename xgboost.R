...
xgb_rec <- recipe(classi_fin ~ ., data = srag_train) %>% 
  ...
  themis::step_rose(classi_fin,seed = 69) #%>% 
# prep()
...
#BSMOTE all_neighbours=FALSE 12:41
#BSMOTE all_neighbours=FALSE 16:00

#BSMOTE all_neighbours=TRUE 16:32
#BSMOTE all_neighbours=TRUE 19>24
...
# saveRDS(xgb_res,"xgb_UPSAMPLE.rds")

xgb_res <- readRDS("xgb_res_artigo1_16-22_final.rds")


# saveRDS(xgb_res,"xgb_BSMOTET.rds")

# saveRDS(xgb_res,"xgb_ADASYN.rds") #SO RODAR ESSA LINHA
# 
# saveRDS(xgb_res,"xgb_SMOTEnc.rds")
...
b <- summary(conf_mat(preds, classi_fin, .pred_class)) %>%
  dplyr::select(-.estimator) %>%
  gt() %>% 
  fmt_number(columns = 2, decimals = 4)

saveRDS(b,"metricas_smote.rds")
