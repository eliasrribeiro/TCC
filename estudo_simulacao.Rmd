...
## Regressão logística com dados originais com tres reamostragens

```{r}
set.seed(149)

data3$classi_fin <- factor(data3$classi_fin,levels=c("COVID-19","Não COVID-19"))
...
data_train_smotenc <-
    recipe(classi_fin ~., data = data_train) %>%
    step_smotenc(classi_fin,seed=(i+69)) %>%
    prep() %>%
    bake(new_data = NULL)
  
model_sim <- glm(classi_fin ~.,data=data_train,family = "binomial")
...
t <-
    threshold_perf(data_test, classi_fin, pred, thresholds = seq(0, 1, by = 0.0025)) %>%
    filter(.metric == "distance") %>%
    filter(.estimate == max(.estimate)) %>%
    pull(.threshold)
  
data_test <- data_test %>% 
    mutate(new_preds = factor(ifelse(pred >= t,"Não COVID-19","COVID-19")))
  
confusionMatrix(as.factor(data_test$new_preds), data_test$classi_fin)



data_categorico_adasyn <- AdasynClassif(classi_fin ~.,as.data.frame(data_train),dist="HEOM")

model_sim <- glm(classi_fin ~.,data=data_categorico_adasyn,family = "binomial")
...
data_categorico_smote <- SmoteClassif(classi_fin ~.,as.data.frame(data_train),dist="HEOM")

model_sim <- glm(classi_fin ~.,data=data_categorico_smote,family = "binomial")
...
g1 <- ggplot(dt_sensibilidade, aes(x = metodo, y = sensibilidade, fill = metodo)) +
  geom_boxplot() +
  theme_minimal() +
...
g3 <- ggplot(dt_sensibilidade, aes(x = metodo, y = sensibilidade, fill = metodo)) +
  geom_boxplot() +
  theme_minimal() +
  theme(legend.position = "none") +
  scale_fill_viridis_d(option = "inferno", direction = -1)  +
...
g5 <- ggplot(dt_sensibilidade, aes(x = metodo, y = sensibilidade, fill = metodo)) +
  geom_boxplot() +
  theme_minimal() +
  theme(legend.position = "none") +
  scale_fill_viridis_d(option = "inferno", direction = -1)  +
...
g7 <- ggplot(dt_sensibilidade, aes(x = metodo, y = sensibilidade, fill = metodo)) +
  geom_boxplot() +
  theme_minimal() +
  theme(legend.position = "none") +
  scale_fill_viridis_d(option = "inferno", direction = -1)  +
