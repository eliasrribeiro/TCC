---
title: "Análises do SIVEP-Gripe para Variantes Gestantes e Puérperas"
...
Note também que há `r dim(dados2)[1]` casos. 
...
dados2$res_igg <-
  ifelse(is.na(dados2$RES_IGG) == TRUE, 0, dados2$RES_IGG)

dados2$res_igm <-
  ifelse(is.na(dados2$RES_IGM) == TRUE, 0, dados2$RES_IGM)

dados2$res_iga <-
  ifelse(is.na(dados2$RES_IGA) == TRUE, 0, dados2$RES_IGA)

dados2$sorologia_SN <-
  ifelse(dados2$res_igg == 1 |
           dados2$res_igm == 1 | dados2$res_iga == 1,
         "sim",
         "não")
...
Há `r dim(dados2[dados2$CS_GESTANT==0, ])[1]` casos com `CS_GESTANT=0`, em que a categoria 0 não tem código no dicionário. 
...
table(dados2$CS_GESTANT, dados2$CS_SEXO)
```
...
table(dados2$PUERPERA, dados2$CS_SEXO)
```
...
dados3 <- dados3 %>% 
  filter(idade_anos > 9 & idade_anos <= 49)
```
...
Ficamos com `r dim(dados4)[1]` casos de gestantes e puérperas, distribuídas nos seguintes grupos de trimestre gestacional e puerpério:

```{r, echo=TRUE, eval= TRUE, message=FALSE, warning =FALSE, error=FALSE}
#tabela de frequência para grupo gestacional
questionr::freq(
  dados4$classi_gesta_puerp,
  cum = FALSE,
  total = TRUE,
  na.last = FALSE,
  valid = FALSE
...
dados5 <- dados5 %>%
  mutate(
    grupos = case_when(
      CS_GESTANT == 1 | CS_GESTANT == 2 |  CS_GESTANT == 3 | CS_GESTANT == 4  ~ "gestante",
      CS_GESTANT == 5 &
        PUERPERA == 1 ~ "puerpera",
      CS_GESTANT == 9 & PUERPERA == 1 ~ "puerpera",
      TRUE ~ "não"
    )
  )

ctable(dados5$variante,dados5$grupos)
```
...
saveRDS(dados5,"dados_newtest.rds")
```
